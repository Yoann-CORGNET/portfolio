...
- `pre-push` runs `pnpm run typecheck` on the whole project and blocks the push if it fails.

To format everything by hand: `pnpm run format`. To check without writing: `pnpm run format:check`.

## Checks

The `ci` workflow runs four jobs in parallel on every pull request. Run them locally first:
