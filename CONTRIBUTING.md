...
- Page routes: `src/app/` (App Router). Each `/projets/<slug>` project has its own route folder with
  its own `page.tsx` and `_components/sections/` — there is no shared `[slug]` template.
- House design system: `src/components/system/` — see `docs/code-quality.md` for the barrel-import
  rule.
- Cross-route chrome: `src/components/shared/` (navbar, footer)
- Section shells used by more than one project page: `src/components/sections/`
- Page-specific sections: colocated per route under `_components/sections/`

Use kebab-case for file names, section components end in `.section.tsx`. Adding a project means
three hand-edits, not one: its own route folder under `src/app/projets/`, a summary entry in the
homepage work grid (`src/app/_components/sections/work-grid.tsx`), and a link in the `CHAIN` in
`src/app/projets/_components/project-transition.tsx`. Nothing checks these agree — a page missing
from the last two is built and publicly reachable but linked from nowhere.
...
Run `typecheck` even though `build` also type-checks now: it's the fast, isolated signal, catching
an error before you wait on a full build.

ESLint runs over the whole of `src/` with no per-path exemptions — there is no vendored code left to
exempt.
...
`sonar-project.properties` sets no exclusions: every file under `src/` is hand-maintained and is
analysed.
...
The code-quality rules for this repo — separation of concerns, no hardcoded design values, single
source of truth, barrel boundaries, naming, type safety, and so on — are defined in
[`docs/code-quality.md`](docs/code-quality.md). Read it and follow it before opening a PR; it also
lists what's tool-enforced versus reviewed by hand, and known gaps.

Keep the docs in sync. Any change that touches structure, routing, data flow, or the conventions
themselves must land with the matching updates to `README.md`, `CLAUDE.md`, `CONTRIBUTING.md`, and
`docs/code-quality.md` in the **same** PR. Documentation has to reflect the change before it is
